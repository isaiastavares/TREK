...
const ru: NotificationLocale = {
  email: {
    footer: 'Вы получили это, потому что у вас включены уведомления в {appName}.',
    manage: 'Управление настройками',
    madeWith: 'Made with',
    openTrek: 'Открыть {appName}',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} приглашает вас объединить планы отпуска. Откройте {appName} для подтверждения.`,
    }),
    vacay_share: (p) => ({
      title: 'Календарь Vacay предоставлен',
      body: `${p.actor} поделился с вами своим календарём отпусков. Откройте {appName}, чтобы посмотреть.`,
    }),
    collection_invite: (p) => ({
      title: 'Приглашение в коллекцию',
      body: `${p.actor} приглашает вас поделиться коллекцией. Откройте {appName} для подтверждения.`,
    }),
    photos_shared: (p) => ({
      title: `${p.count} фото`,
  ...
  passwordReset: {
    ...
    body: 'Мы получили запрос на сброс пароля вашего аккаунта {appName}. Нажмите кнопку ниже, чтобы установить новый пароль.',

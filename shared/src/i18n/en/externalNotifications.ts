...
const en: NotificationLocale = {
  email: {
    footer: 'You received this because you have notifications enabled in {appName}.',
    manage: 'Manage preferences in Settings',
    madeWith: 'Made with',
    openTrek: 'Open {appName}',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} invited you to fuse vacation plans. Open {appName} to accept or decline.`,
    }),
    vacay_share: (p) => ({
      title: 'Vacay Calendar Shared',
      body: `${p.actor} shared their vacation calendar with you. Open {appName} to view it.`,
    }),
    collection_invite: (p) => ({
      title: 'Collection invite',
      body: `${p.actor} invited you to share a collection. Open {appName} to accept or decline.`,
    }),
    photos_shared: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Hi',
    body: 'We received a request to reset the password for your {appName} account. Click the button below to set a new password.',

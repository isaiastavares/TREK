...
const pl: NotificationLocale = {
  email: {
    footer: 'Otrzymałeś/aś tę wiadomość, ponieważ masz włączone powiadomienia w {appName}.',
    manage: 'Zarządzaj preferencjami w ustawieniach',
    madeWith: 'Made with',
    openTrek: 'Otwórz {appName}',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} zaprosił Cię do połączenia planów urlopowych. Otwórz {appName}, aby zaakceptować lub odrzucić.`,
    }),
    vacay_share: (p) => ({
      title: 'Kalendarz Vacay udostępniony',
      body: `${p.actor} udostępnił Ci swój kalendarz urlopów. Otwórz {appName}, aby go zobaczyć.`,
    }),
    collection_invite: (p) => ({
      title: 'Zaproszenie do kolekcji',
      body: `${p.actor} zaprosił Cię do udostępnienia kolekcji. Otwórz {appName}, aby zaakceptować lub odrzucić.`,
    }),
    photos_shared: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Cześć',
    body: 'Otrzymaliśmy prośbę o zresetowanie hasła do Twojego konta {appName}. Kliknij przycisk poniżej, aby ustawić nowe hasło.',

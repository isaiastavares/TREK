...
const uk: NotificationLocale = {
  email: {
    footer: 'Ви отримали це, оскільки увімкнули сповіщення в {appName}.',
    manage: 'Керувати налаштуваннями у Налаштуваннях',
    madeWith: 'Made with',
    openTrek: 'Відкрити {appName}',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} запрошує вас об'єднати плани відпустки. Відкрийте {appName}, щоб прийняти або відхилити.`,
    }),
    vacay_share: (p) => ({
      title: 'Календар Vacay надано',
      body: `${p.actor} поділився з вами своїм календарем відпусток. Відкрийте {appName}, щоб переглянути.`,
    }),
    collection_invite: (p) => ({
      title: 'Запрошення до колекції',
      body: `${p.actor} запрошує вас поділитися колекцією. Відкрийте {appName}, щоб прийняти або відхилити.`,
    }),
    photos_shared: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Привіт',
    body: 'Ми отримали запит на скидання пароля вашого облікового запису {appName}. Натисніть кнопку нижче, щоб встановити новий пароль.',

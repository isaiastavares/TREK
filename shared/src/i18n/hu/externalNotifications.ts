...
const hu: NotificationLocale = {
  email: {
    footer: 'Ezt az értesítést azért kaptad, mert engedélyezted az értesítéseket a {appName}-ben.',
    manage: 'Beállítások kezelése',
    madeWith: 'Made with',
    openTrek: '{appName} megnyitása',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} meghívott a nyaralási tervek összevonásához. Nyissa meg a {appName}-et az elfogadáshoz vagy elutasításhoz.`,
    }),
    vacay_share: (p) => ({
      title: 'Vacay naptár megosztva',
      body: `${p.actor} megosztotta veled a szabadságnaptárát. Nyissa meg a {appName}-et a megtekintéshez.`,
    }),
    collection_invite: (p) => ({
      title: 'Gyűjtemény meghívó',
      body: `${p.actor} meghívott egy gyűjtemény megosztására. Nyissa meg a {appName}-et az elfogadáshoz vagy elutasításhoz.`,
    }),
    photos_shared: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Szia',
    body: 'Kérést kaptunk a {appName}-fiókod jelszavának visszaállítására. Kattints az alábbi gombra az új jelszó beállításához.',

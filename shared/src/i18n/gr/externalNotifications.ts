...
const gr: NotificationLocale = {
  email: {
    footer: 'Λάβατε αυτό το μήνυμα επειδή έχετε ενεργοποιήσει τις ειδοποιήσεις στο {appName}.',
    manage: 'Διαχείριση προτιμήσεων στις Ρυθμίσεις',
    madeWith: 'Δημιουργήθηκε με',
    openTrek: 'Άνοιγμα {appName}',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `Ο/Η ${p.actor} σας προσκάλεσε να συγχωνεύσετε τα σχέδια διακοπών σας. Ανοίξτε το {appName} για να αποδεχτείτε ή να απορρίψετε.`,
    }),
    vacay_share: (p) => ({
      title: 'Κοινοποίηση Ημερολογίου Vacay',
      body: `Ο/Η ${p.actor} κοινοποίησε το ημερολόγιο διακοπών του/της σε εσάς. Ανοίξτε το {appName} για να το δείτε.`,
    }),
    collection_invite: (p) => ({
      title: 'Πρόσκληση σε συλλογή',
      body: `Ο/Η ${p.actor} σας προσκάλεσε να μοιραστείτε μια συλλογή. Ανοίξτε το {appName} για να αποδεχτείτε ή να απορρίψετε.`,
    }),
    photos_shared: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Γεια σας',
    body: 'Λάβαμε ένα αίτημα επαναφοράς του κωδικού πρόσβασης για τον λογαριασμό σας στο {appName}. Κάντε κλικ στο παρακάτω κουμπί για να ορίσετε νέο κωδικό πρόσβασης.',

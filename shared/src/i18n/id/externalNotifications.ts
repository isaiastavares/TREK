...
const id: NotificationLocale = {
  email: {
    footer: 'Anda menerima ini karena Anda telah mengaktifkan notifikasi di {appName}.',
    manage: 'Kelola preferensi di Pengaturan',
    madeWith: 'Dibuat dengan',
    openTrek: 'Buka {appName}',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} mengundang Anda untuk menggabungkan rencana liburan. Buka {appName} untuk menerima atau menolak.`,
    }),
    vacay_share: (p) => ({
      title: 'Kalender Vacay Dibagikan',
      body: `${p.actor} membagikan kalender cutinya dengan Anda. Buka {appName} untuk melihatnya.`,
    }),
    collection_invite: (p) => ({
      title: 'Undangan koleksi',
      body: `${p.actor} mengundang Anda untuk berbagi koleksi. Buka {appName} untuk menerima atau menolak.`,
    }),
    photos_shared: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Halo',
    body: 'Kami menerima permintaan untuk menyetel ulang kata sandi akun {appName} Anda. Klik tombol di bawah untuk menetapkan kata sandi baru.',
